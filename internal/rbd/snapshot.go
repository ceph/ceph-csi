...
func cleanUpSnapshot(
	...
	if err != nil {
		if !errors.Is(err, ErrSnapNotFound) && !errors.Is(err, ErrImageNotFound) {
	...
	if rbdVol != nil {
		err := rbdVol.deleteSnapshot(ctx, rbdSnap)
		if err != nil {
			if !errors.Is(err, ErrSnapNotFound) && !errors.Is(err, ErrImageNotFound) {
				log.ErrorLog(ctx, "failed to delete clone snapshot %q: %v", rbdSnap, err)
