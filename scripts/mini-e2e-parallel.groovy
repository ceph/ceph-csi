	...
	try {
		...
		stage('run e2e') {
			timeout(time: 90, unit: 'MINUTES') {
				def branches = [:]
				def test_types = ["cephfs", "rbd", "nfs", "nvmeof"]
				...
				for (t in test_types) {
					...
					switch (test_type) {
						case "cephfs":
							flags = "--test-cephfs=true --test-rbd=false --test-nfs=false --test-nvmeof=false"
							break
						case "rbd":
							flags = "--test-rbd=true --test-cephfs=false --test-nfs=false --test-nvmeof=false"
							break
						case "nfs":
							flags = "--test-nfs=true --test-cephfs=false --test-rbd=false --test-nvmeof=false"
							break
						case "nvmeof":
							flags = "--test-nvmeof=true --test-nfs=false --test-cephfs=false --test-rbd=false"
							break
					}

					branches[test_type] = {
						ssh "cd /opt/build/go/src/github.com/ceph/ceph-csi && make run-e2e E2E_ARGS=\"--delete-namespace-on-failure=false --cephcsi-namespace=ceph-csi-test${test_type} ${flags}\""
					}
				}
