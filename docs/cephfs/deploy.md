...
Ceph-CSI supports optionally restricting CephFS volume access to specific Kubernetes
service accounts. When configured, only pods running with the allowed service account
can mount the volume. This feature uses CephFS subvolume metadata to store the
restriction and the CSI `podInfoOnMount` mechanism to identify the pod's service
account during mount.

### How it works

1. A user sets the `.cephfs.csi.ceph.com/serviceaccount` metadata on a
   CephFS subvolume to specify the allowed service account name.
